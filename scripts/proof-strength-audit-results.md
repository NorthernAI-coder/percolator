...
```text
SUMMARY: 25 passed, 0 failed/timeout (0 timeout) out of 25
```
...
| Fail | 0 |
| Timeout | 0 |
| Total wall-clock harness time | 657s |
| Slowest harness | `proof_v13_account_b_chunk_either_advances_or_fails_closed` |
| Slowest harness time | 132s |

## Harness Timings

| Harness | Time | Status |
|---|---:|---|
| `proof_v13_account_b_chunk_either_advances_or_fails_closed` | 132s | PASS |
| `proof_v13_rebalance_reduce_position_preserves_senior_claims_and_reduces_risk` | 116s | PASS |
| `proof_v13_hlock_allows_pure_risk_reducing_trade_with_principal_margin` | 104s | PASS |
| `proof_v13_bankrupt_liquidation_consumes_insurance_before_social_loss` | 56s | PASS |
...
| `proof_v13_resolved_positive_payout_snapshot_is_order_stable` | 27s | PASS |
...
| `proof_v13_equity_active_accrual_advances_at_most_one_bounded_segment` | 6s | PASS |
...
| File | Harnesses |
|---|---:|
| `tests/proofs_v13.rs` | 25 |

Strength indicators:

| Check | Result |
|---|---:|
| Harnesses over v13 production code | 25 |
| Harnesses with symbolic inputs or symbolic branch choices | 9 |
| Harnesses with `kani::cover!` reachability checks | 9 |
...
|---|---|
| Non-vacuity | No confirmed vacuous harnesses found. Cover checks exercise h-min/h-max, stale set/clear, hidden-leg rejection, B-chunk progress paths, and both covered rebalance reduction paths. |
...
The Rust suite currently covers 50 wide-math unit tests and 38 v13 spec tests.
...
payout snapshot fairness, rebalance strict risk-progress with senior-claim
preservation, and permissionless account-local progress. The retired v12 slab
proofs no longer apply after the architectural cutover.
