...
| 2 | machine-checked global no-LoF transition composition theorem | DONE (decomposed) | GlobalValidState named (validate_shape ∧ per-account validate_with_market); boundary_audit.py proves it holds at every committed Ok-exit (56/56); validator semantics Kani-proven. The "exact frame ∧ value flow" layer is the per-op frames (16 direct + composition frames below). |
...
| 9 | no-LoF transition-class roster (public path -> proof source) | DONE (enforced) | scripts/lof_transition_class_roster.py partitions all 56 public `*_not_atomic` entrypoints into 10 transition classes, each with a present proof-source artifact (DIRECT_FRAME / WHOLE_BODY_COMPOSITION / KERNEL_VALUE / FLOW_VALIDATOR / CLOSURE) atop the universal GlobalValidState floor (boundary_audit 56/56). Fails the build on any unclassified/double-counted entrypoint or missing artifact. |
